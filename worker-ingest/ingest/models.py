"""Mirrors the `companies` / `jobs` / `job_sources` tables owned by
core-api's Alembic migrations (core-api/app/models/job.py). worker-ingest
only ever writes to these three tables, so it declares thin models against
the same schema rather than sharing a package across two separate uv
projects/venvs.
"""
